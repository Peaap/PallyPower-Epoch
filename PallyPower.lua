local AceAddon3 = LibStub and LibStub("AceAddon-3.0", true)
if AceAddon3 then
  PallyPower = AceAddon3:NewAddon("PallyPower", "AceConsole-3.0", "AceEvent-3.0", "AceTimer-3.0", "AceBucket-3.0")
else
  PallyPower = {}
end
...
function PallyPower:OnInitialize()
	if LibStub and LibStub("AceDB-3.0", true) then
		self.db = LibStub("AceDB-3.0"):New("PallyPowerDB", { profile = PALLYPOWER_DEFAULT_VALUES }, true)
	end
	if self.RegisterChatCommand then
		self:RegisterChatCommand("pp", "SlashHandler")
	end
	self.player = UnitName("player")
	self.opt = (self.db and self.db.profile) or PALLYPOWER_DEFAULT_VALUES
	self:ScanInventory()
	self:CreateLayout()
	...
	if AceConfig and AceConfigDialog and PallyPower.options then
		AceConfig:RegisterOptionsTable("PallyPower", PallyPower.options)
		if AceDBOptions and self.db then
			self.profilesOptions = AceDBOptions:GetOptionsTable(self.db)
			AceConfig:RegisterOptionsTable("PallyPower-Profiles", self.profilesOptions)
			AceConfigDialog:AddToBlizOptions("PallyPower", "PallyPower")
			AceConfigDialog:AddToBlizOptions("PallyPower-Profiles", "Profiles", "PallyPower")
		else
			AceConfigDialog:AddToBlizOptions("PallyPower", "PallyPower")
		end
	end
...
function PallyPower:SlashHandler()
	PallyPowerConfig_Toggle()
end

if not PallyPower.Debug then
  function PallyPower:Debug()
    -- no-op during Ace3 migration
  end
end
...
function PallyPower:OnEnable()
	...
	if self.ScheduleRepeatingTimer then
		...
		if self.timers.inventory then self:CancelTimer(self.timers.inventory, true) end
		self.timers.inventory = self:ScheduleRepeatingTimer("InventoryScan", 60)
	end
	self:UpdateRoster()
	self:BindKeys()
end
...
function PallyPowerGrid_NormalBlessingMenu(btn, mouseBtn, pname, class)
	...
	if (mouseBtn == "LeftButton") then
		local tempoptions = {
			type = "group",
			args = {
				-- open AceGUI assignment dialog for per-player blessing selection
			}
		}
		local pre, suf
		...
		PallyPower:OpenAssignDialog(pname, class)
...
function PallyPowerConfigGrid_Update()
	...
	if PallyPowerConfigFrame:IsVisible() then
		...
		local cfgScale = tonumber(PallyPower.opt and PallyPower.opt.configscale) or (PALLYPOWER_DEFAULT_VALUES and PALLYPOWER_DEFAULT_VALUES.configscale) or 1
		PallyPower.opt.configscale = cfgScale
		PallyPowerConfigFrame:SetScale(cfgScale)
		for i, name in pairs(SyncList) do
			...
			for id = 1, (PallyPower.NumBlessings or 5) do
    			...
    			-- show Salvation (5) skill; only hide unused 6th slot
			getglobal(fname.."Icon"..6):Hide()
			getglobal(fname.."Skill"..6):Hide()
	...
	if not type then
		...
		if PallyPower:CheckRaidLeader(self.player) then
			...
			for name in pairs(AllPallys) do
				local blessings
				for i = 1, (PallyPower.NumBlessings or 5) do
					list[i] = 0
				end
				for id = 1, PALLYPOWER_MAXCLASSES do
...
function PallyPower:PerformCycle(name, class, skipzero)
	...
	local maxb = (PallyPower.NumBlessings or 4)
	local prev = 0
	...
	-- wrap only if nothing new found
	if cur == prev then
		if skipzero then
			cur = 1
		else
...
function PallyPower:PerformCycleBackwards(name, class, skipzero)
	...
	if not PallyPower_Assignments[name][class] then
		...
		cur = PallyPower_Assignments[name][class]
		if (cur == 0) or (skipzero and cur == 1) then cur = maxb end
	end
	prev = cur
...
function PallyPower:CanBuff(name, test)
    if not AllPallys[name] then return false end
...
function PallyPower:OpenAssignDialog(pname, class)
    local AceGUI = LibStub and LibStub("AceGUI-3.0", true)
    if not AceGUI then return end
    if self.assignFrame then
        self.assignFrame:Release()
        self.assignFrame = nil
    end
    local frame = AceGUI:Create("Frame")
    frame:SetTitle(string.format("Assign Blessings: %s", pname))
    frame:SetLayout("Flow")
    frame:SetWidth(360)
    frame:SetHeight(420)
    frame:SetCallback("OnClose", function(widget) AceGUI:Release(widget); self.assignFrame = nil end)
    self.assignFrame = frame
    ...
    for pally, _ in pairs(AllPallys) do
        local control = PallyPower:CanControl(pally)
        local group = AceGUI:Create("InlineGroup")
        group:SetTitle(pally)
        ...
        for index, blessing in ipairs(PallyPower.Spells) do
            if PallyPower:CanBuff(pally, index) then
                list[tostring(index)] = blessing
            end
        end
        dd:SetList(list)
        local cur = 0
        if PallyPower_NormalAssignments[pally] and PallyPower_NormalAssignments[pally][class] and PallyPower_NormalAssignments[pally][class][pname] then
            cur = PallyPower_NormalAssignments[pally][class][pname] or 0
        end
        dd:SetValue(tostring(cur))
        dd:SetDisabled(not control)
        dd:SetCallback("OnValueChanged", function(widget, event, key)
            local val = tonumber(key) or 0
            if control then
                SetNormalBlessings(pally, class, pname, val)
            end
        end)
        group:AddChild(dd)
    end
...
function PallyPower:NeedsBuff(class, test, playerName)
    ...
    if self.opt.smartbuffs then
        ...
        -- avoid Greater Salvation on tanks/classes (allow normal per-player)
        if (not playerName) and test == 5 and (class == 1 or class == 4 or class == 5) then
            return false
        end
...
local function PP_SafeGetSpellInfo(x) local ok,a,b,c,d,e,f,g,h = pcall(GetSpellInfo, x); if ok then return a,b,c,d,e,f,g,h end return nil end
...
function PallyPower:ScanSpells()
	...
	if (class == "PALADIN") then
		local RankInfo = {}
		for i = 1, (PallyPower.NumBlessings or 5) do -- find max spell ranks
			local spellName, spellRank = PP_SafeGetSpellInfo(PallyPower.GSpells[i])
			if not spellName then -- fallback to lower blessings
				spellName, spellRank = PP_SafeGetSpellInfo(PallyPower.Spells[i])
			end
...
--    local name = UnitName("player")
	...
	s = ""
	for i = 1, (PallyPower.NumBlessings or 4) do
    ...
    local count = #AssignList
    if count > 0 then
        local offset = 1
        repeat
            self:SendMessage("NASSIGN " .. table.concat(AssignList, "@", offset, math.min(offset + 4, count)))
            offset = offset + 5
        until offset > count
    end
...
function PallyPower:UpdateRoster()
	...
	if self.CancelTimer and self.timers and self.timers.buttonsUpdate then
		self:CancelTimer(self.timers.buttonsUpdate, true)
		self.timers.buttonsUpdate = nil
	end
...
--						end
	...
	if num > 0 and PP_IsPally and self.ScheduleRepeatingTimer then
		self.timers = self.timers or {}
		if self.timers.buttonsUpdate then self:CancelTimer(self.timers.buttonsUpdate, true) end
		self.timers.buttonsUpdate = self:ScheduleRepeatingTimer("ButtonsUpdate", 2.0)
	end
...
function PallyPower:UpdateLayout()
	...
	if InCombatLockdown() then return false end
	-- sanitize scale in case saved var is corrupted
	local scale = tonumber(self.opt and self.opt.buffscale) or (PALLYPOWER_DEFAULT_VALUES and PALLYPOWER_DEFAULT_VALUES.buffscale) or 1
	self.opt.buffscale = scale
	PallyPowerFrame:SetScale(scale)
	
	if self.opt.layout == "Standard" then
	
		local rows = (self.opt.display and self.opt.display.rows) or 11
		local columns = (self.opt.display and self.opt.display.columns) or 1
		local gapping = (self.opt.display and self.opt.display.gapping) or -1
		local buttonWidth = (self.opt.display and self.opt.display.buttonWidth) or 100
		local buttonHeight = (self.opt.display and self.opt.display.buttonHeight) or 34
...
local WisdomPallys, MightPallys, KingsPallys,  SancPallys, SalvPallys = {}, {}, {}, {}, {}

function PallyPower:AutoAssign()

	PallyPowerConfig_Clear()
    WisdomPallys, MightPallys, KingsPallys,  SancPallys, SalvPallys = {}, {}, {}, {}, {}	
...
function PallyPower:CalcSkillRanks1(name)
    local wisdom, might, kings, sanct, salv
    ...
    if AllPallys[name][5] then
        salv  = tonumber(AllPallys[name][5].rank)
    else
        salv = 0
    end
    
    return wisdom, might, kings, sanct, salv
end
...
function PallyPower:AutoAssignBlessings()
	...
	for name in pairs(AllPallys) do	
		...
		local wisdom, might, kings, sanct, salv = PallyPower:CalcSkillRanks1(name) 
		--self:Print("Adding")
...
function PallyPower:BuffSelections(buff, class, pallys)
    ...
    if buff == 5 then t = SalvPallys end
...
--				testtalent = t[i].spelltalents
	...
	--end
	if Buffer ~= "" then
			if not PallyPower_Assignments[Buffer] then
